...
При виклику функції відбуваються такі процеси:
  1. Поточне значення IP (адреса, куди треба повернутися) зберігається на стеці
  2. IP отримує нове значення — адресу викликаної функції
  3. Після завершення функції, адреса повернення вилучається зі стеку та знову записується в IP

## Структура стеку при виклику функцій
Стек містить не лише адреси повернення, але й локальні змінні та інші дані. При цьому адреси повернення не обов'язково знаходяться на вершині стеку.

### Приклад роботи зі стеком
...
void функція_A() {
    int x = 5;
    функція_B();
    // продовжити виконання після повернення з функції_B
}
...
Коли `функція_A` викликає `функція_B`, стек може виглядати так:

```
| y = 10              | ← Вершина стеку (змінна функції_B)
| Адреса повернення B | (куди повернутися в функцію_A)
| x = 5               | (змінна функції_A)
| Адреса повернення A | (куди повернутися після функції_A)
| ...                 |
```

## Чому вершина стеку не може замінити IP?
Якби вершина стеку використовувалась як лічильник команд, процесор спробував би інтерпретувати значення `10` (змінна `y`) як адресу наступної інструкції. Це призвело б до спроби виконати дані як код, що викликало б критичну помилку в роботі програми.

## Висновок
IP і стек виконують різні функції в архітектурі комп'ютера:
- **IP** — постійно вказує на наступну інструкцію для виконання
- **Стек** — зберігає різноманітні дані, включаючи адреси повернення, локальні змінні та параметри функцій

Саме тому лічильник команд повинен бути окремим спеціалізованим регістром, незалежним від вмісту та стану стеку пам'яті.
